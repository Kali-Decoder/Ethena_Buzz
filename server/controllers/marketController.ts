...
export const createMarket = async (req: Request, res: Response): Promise<void> => {
  try {
    const { title, description, type, minRange, maxRange, startTime, endTime, txHash } = req.body;
...
export const resolveMarket = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const { outcome, outcomeValue, txHash } = req.body;
    const resolver = req.body.userAddress; // From auth middleware
    
    const market = await marketService.resolveMarket(id, resolver, outcome, outcomeValue);
...
export const getUserMarkets = async (req: Request, res: Response): Promise<void> => {
  try {
