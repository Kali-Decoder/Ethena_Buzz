...
export const createMarket = async (params: MarketCreateParams): Promise<IMarket> => {
  const {
    title,
    description,
    creator,
...
export const getAllMarkets = async (filters: MarketFilters = {}): Promise<IMarket[]> => {
  const query: any = {};
...
export const updateMarket = async (id: string, updates: Partial<IMarket>): Promise<IMarket | null> => {
...
export const resolveMarket = async (
  ...
  if (market.status === MarketStatus.RESOLVED) throw new Error('Market already resolved');
  if (new Date() < market.endTime) throw new Error('Cannot resolve market before end time');
