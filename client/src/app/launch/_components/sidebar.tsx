    ...
    <>
      <div className={`sidebar ${isOpen ? 'open' : ''}`}>
        <div className="sidebar-content bg-transparent">
          {isOpen && <button className="sidebar-toggle" onClick={closeSideBar}>
            ▶
          </button>}
          ...
          <div className='mt-20'>
            <div className='flex flex-col gap-y-2'>
              <p className='text-md font-bold'>#00{filterData?.id} <span className='text-blue-400'>{`{${filterData?.category}}`} </span> <span className='text-green-400'>{`@ ${filterData?.name}`}</span></p>
              <p className='font-bold text-xl text-white'>{filterData?.description}</p>
            ...
            <label htmlFor="value" className="mb-2 block text-lg font-medium mt-10">Predict Value</label>
            ...
            <div className="max-w-sm mt-4">
              <label htmlFor="token" className="mb-2 block text-lg font-medium">Enter Amount</label>
              <div className="relative">
                <input type="number"
                  value={investment}
                  onChange={(e) => setInvestment(e.target.value)} id="amount" name="inline-add-on" className="block w-full bg-transparent rounded-md border border-gray-200 py-3 px-4 pl-4 text-lg shadow-sm outline-none focus:z-10 focus:border-blue-500 focus:ring-blue-500" placeholder="$ 00" />
                <div className="absolute inset-y-0 right-0 flex items-center px-3 text-gray-500">
                  ...
                  <select id="token" name="token" className="block w-full rounded-md p-2 border text-white bg-transparent ">
                    <option>USDC</option>
                    <option>BUZZ</option>
                    <option>USDT</option>
                    <option>DAI</option>
